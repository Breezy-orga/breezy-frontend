        ...
        <div className="relative">
          ...
          {userMenuOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-lg z-40 animate-fade-in">
              ...
              >
                {theme === 'dark' ? <MdLightMode className="text-xl" /> : <MdDarkMode className="text-xl" />} {theme === 'dark' ? 'Mode clair' : 'Mode sombre'}
              </button>
...
function Poll({ poll }: { poll: Poll }) {
  ...
  return (
    <div className="mt-3">
      <div className="font-semibold text-gray-800 mb-1">{poll.question}</div>
      {poll.options.map((opt: string, i: number) => (
        <div key={opt} className="flex items-center gap-2 mb-1">
          <div className="w-2/3 bg-gray-100 rounded-full h-3 overflow-hidden">
            <div className="bg-blue-400 h-3 rounded-full" style={{ width: `${total ? (poll.votes[i] / total) * 100 : 0}%` }}></div>
          </div>
          <span className="text-sm text-gray-700">{opt}</span>
          <span className="text-xs text-gray-500">{poll.votes[i]} vote{poll.votes[i] > 1 ? 's' : ''}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md p-6 mb-6 border border-gray-100 dark:border-gray-800 animate-fade-in relative">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          <div className="relative">
            <Image src={post.user.avatar} alt="Photo profil" width={40} height={40} className="rounded-full object-cover border border-gray-200" />
            {post.user.isOnline && <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 rounded-full border-2 border-white"></span>}
          </div>
          <div>
        ...
        <div className="flex gap-2">
          ...
          <div className="relative">
            <button className="flex items-center gap-1 text-gray-500 hover:text-blue-600 px-2 py-1 rounded transition" title="Partager" onClick={() => setShowShare(v => !v)}>
              <MdShare className="text-xl" />
            </button>
            {showShare && (
              <div className="absolute right-0 mt-2 w-44 bg-white border border-gray-200 rounded-xl shadow-lg z-40 animate-fade-in">
                <button className="w-full text-left px-4 py-2 hover:bg-blue-50 transition flex items-center gap-2"><MdLink className="text-xl" /> Copier le lien</button>
                <button className="w-full text-left px-4 py-2 hover:bg-blue-50 transition flex items-center gap-2"><MdShare className="text-xl" /> Partager sur X</button>
                <button className="w-full text-left px-4 py-2 hover:bg-blue-50 transition flex items-center gap-2"><MdSend className="text-xl" /> Envoyer en message</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex gap-2">
        ...
        <button className="flex items-center gap-1 hover:text-blue-600 transition group" onClick={() => setShowComment(v => !v)}>
          <MdChatBubbleOutline className="text-xl" /> {post.comments}
        </button>
        <button className="flex items-center gap-1 hover:text-blue-600 transition group" onClick={() => setShowShare(v => !v)}>
          <MdShare className="text-xl" />
        </button>
      </div>
      {showComment && (
        <div className="mt-4 flex gap-2 items-start animate-fade-in">
          <Image src="/pp1.jpg" alt="Moi" width={32} height={32} className="rounded-full object-cover border border-gray-200" />
          <textarea value={comment} onChange={e => setComment(e.target.value)} className="flex-1 border border-gray-200 rounded-xl p-2 resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all min-h-[40px] text-gray-900 placeholder-gray-400 bg-gray-50" placeholder="Ajouter un commentaire..." />
        ...
        <div className="flex flex-col gap-3">
          {fakeTrends.map(t => (
            <div key={t.tag} className="flex items-center gap-2">
              <span className="text-blue-600 font-semibold cursor-pointer hover:underline">{t.tag}</span>
              <span className="text-xs text-gray-500">{t.count} posts</span>
            </div>
          ))}
        </div>
      </div>
      <div>
        <h2 className="text-xl font-bold mb-4 text-gray-900">Suggestions d&apos;amis</h2>
        <div className="flex flex-col gap-4">
          {fakeStories.slice(2).map(f => (
            <div key={f.username} className="flex items-center gap-3">
              <Image src={f.avatar} alt="Photo profil" width={32} height={32} className="rounded-full object-cover border border-gray-200" />
              <span className="text-gray-900 font-medium">@{f.username}</span>
              <button className="ml-auto bg-blue-100 text-blue-700 px-3 py-1 rounded-lg text-xs font-semibold hover:bg-blue-200 transition">Suivre</button>
            </div>
          ))}
        </div>
      </div>
        ...
        {navItems.map(item => (
          <button
            ...
            className={`flex items-center gap-3 px-4 py-2 rounded-lg transition-all duration-150 group
              ${active === item.key ? 'bg-blue-100 text-blue-700 shadow font-bold scale-[1.04]' : 'text-gray-700 hover:text-blue-700 hover:bg-blue-50'}
            `}
          >
            <item.icon className={`text-xl transition-all duration-150 ${active === item.key ? 'text-blue-700' : 'text-gray-400 group-hover:text-blue-700'}`} />
